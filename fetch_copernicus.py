...
def main():
    ...
    try:
        phys = cm.open_dataset(
            ...
            variables=["zos"],
            start_datetime=start, end_datetime=end, **auth, **BOX)
    except Exception:
        print("Failed to load SSH dataset", file=sys.stderr)
        traceback.print_exc()
        raise
    ...
    try:
        theta = cm.open_dataset(
            ...
            minimum_depth=0, maximum_depth=250, **auth, **BOX)
    except Exception:
        print("Failed to load temperature dataset", file=sys.stderr)
        traceback.print_exc()
        raise
